/*
 * LWPolyline.h
 *
 *  Created on: May 24, 2026
 *      Author: Gemini CLI
 */

#ifndef ENTITIES_LWPOLYLINE_H_
#define ENTITIES_LWPOLYLINE_H_

#include <vector>
#include <string>
#include <pugixml.hpp>
#include "document/Entity.h"

namespace dxf {

class LWPolyline final : public Entity {
private:
    struct Vertex {
        double x = 0.0;
        double y = 0.0;
        double bulge = 0.0;
        double start_width = 0.0;
        double end_width = 0.0;
    };
    std::vector<Vertex> vertices_;
    int flags_ = 0;              // 70: 1 = closed
    double constant_width_ = 0.0; // 43
    double elevation_ = 0.0;      // 38
    double thickness_ = 0.0;      // 39
    Point_ extrusion_direction_ = {0.0, 0.0, 1.0}; // 210, 220, 230

protected:
    virtual void calc_bounding_box() override;

public:
    LWPolyline(const std::vector<Group> &properties);
    virtual ~LWPolyline() {}

    virtual std::string to_json() override;
    virtual void write_to_json_writer(rapidjson::Writer<rapidjson::StringBuffer>& writer) override;
    virtual void to_svg(pugi::xml_node& svg_node) override;
};

} /* namespace dxf */

#endif /* ENTITIES_LWPOLYLINE_H_ */
